...
{
    if (!_collectionView) {
        itemW = ((ACMedia_ScreenWidth-30) - 5 * space)/4;
        UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc]init];
        layout.itemSize = CGSizeMake(itemW, itemW);
        layout.minimumInteritemSpacing = space;
        layout.minimumLineSpacing = space;
        layout.sectionInset = UIEdgeInsetsMake(space/2, space, space/2, space);
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, ACMedia_ScreenHeight, ACMedia_ScreenWidth-30, itemW + space/2 * 2) collectionViewLayout:layout];
