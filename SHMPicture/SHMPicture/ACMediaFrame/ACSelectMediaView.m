...
- (void)configureCollectionView {
    ...
    layout.itemSize = CGSizeMake(self.width/3, self.width/3);
...
+ (CGFloat)defaultViewHeight {
    return (ACMedia_ScreenWidth-30)/3;
}

#pragma mark -  Collection View DataSource

- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if(_mediaArray.count==9){
...
- (void)layoutCollection: (NSArray *)images {
    ...
    NSInteger maxRow;
    if(_mediaArray.count==9){
    maxRow = (allImageCount - 1) / 3;
    }else{
    maxRow = (allImageCount - 1) / 3 + 1;
    }
    _collectionView.height = maxRow * (ACMedia_ScreenWidth-30)/3;
